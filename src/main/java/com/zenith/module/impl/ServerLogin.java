...
public class ServerLogin extends Module {
    ...
    @Override
    public void onEnable() {  // 或其他初始化方法
        //System.out.println("[DEBUG] ServerLogin loaded!");
    }
    @Override
    public List<EventConsumer<?>> registerEvents() {
        //System.out.println("[CRITICAL] registerEvents() is being called!");
        //System.out.println("[CRITICAL] SystemChatEvent listener being registered");
        return List.of(
    ...
    public boolean enabledSetting() {
        boolean b1 = CONFIG.authentication.serverLoginRequired;
        Config.Authentication.AccountType accountType = CONFIG.authentication.accountType;
        //info("accountType="+accountType.toString());
        boolean b2 = (accountType==Config.Authentication.AccountType.OFFLINE);
        if(b1){
           //info("b1 is true");           
        }
        if(b2){
    ...
    public static boolean staticenabledSetting() {
        boolean b1 = CONFIG.authentication.serverLoginRequired;
        Config.Authentication.AccountType accountType = CONFIG.authentication.accountType;
        //info("accountType="+accountType.toString());
        boolean b2 = (accountType==Config.Authentication.AccountType.OFFLINE);
    ...
    private void handleSystemChat(SystemChatEvent event) {
        //info("in handleSystemChat=");
        if (event == null) {
            //info("[DEBUG] Event is null!");
            return;
        }
        ...
        if (msg.contains("/login") && (currentState == State.DONE || currentState == State.AUTHENTICATING)) {
        ...
        if (msg.contains("/register") && (currentState == State.DONE || currentState == State.AUTHENTICATING)) {
        ...
        // ===== SUCCESS MESSAGE =====
        if (msg.contains("you are now logged in") || msg.contains("successfully logged in") || msg.contains("successfully registered") || msg.contains("has been registered") || msg.contains("logged in!") || msg.contains("成功登录")) {
